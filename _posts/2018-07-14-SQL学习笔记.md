...
统计学是关于数据的收集、分析、处理、解释数据并从数据中得出结论的科学。而在当下几TB的数据集实例的时代，传统的EXCEL数据管理模式已经无法支持此规模的数据管理，数据库管理（Databse Management）成为比较重要的一个环节，其中包括MySQL、Oracle、Access等数据库管理系统。数据分析最重要的工作莫过于提取和管理数据，本例以Dognition公司的数据库为例，简要介绍一下SQL语句的基本语法。
